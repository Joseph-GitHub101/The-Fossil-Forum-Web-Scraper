...
url3 = "http://www.thefossilforum.com/index.php?/topic/101415-a-few-riker-mounts-with-specimens-from-the-aquia-formation-of-maryland-and-the-nanjemoy-formation-of-virginia/"
url4 = "http://www.thefossilforum.com/index.php?/topic/99130-cretaceous-new-jersey-fossils/"
url5 = "http://www.thefossilforum.com/index.php?/topic/101728-the-vertebra-appreciation-thread-show-some-back-bone/"
            ...
            if p_tag.find('img'):
                ...
                for i in thumbnails:
                    ...
                    if i.find_parent('a') != None:
                        ...
                        # make sure that the image URL is not that of an emoticon, reaction, profile, award, or mailto link
                        if href != None and re.search(('/uploads/reactions/'), href) == None and re.search(('/uploads/emoticons/'), href) == None and re.search(('/uploads/awards/'), href) == None and re.search(('/profile/'), href) == None and re.search(('mailto:'), href) == None:
                            links.append(href)
                    ...
                    # if the parent with the href link doesn't exist, only then do we settle for the src thumbnail
                        ...
                        src = str(i.get('src'))
                        # make sure that the image URL is not that of an emoticon, reaction, profile, award, or mailto link
                        if src != None and re.search(('/uploads/reactions/'), src) == None and re.search(('/uploads/emoticons/'), src) == None and re.search(('/uploads/awards/'), src) == None and re.search(('/profile/'), src) == None and re.search(('mailto:'), src) == None:
                            links.append(src)
...
def main(url, path):
    ...
    for img in final_imgs:
        # use a random string of 8 numbers, so that each photo file has a unique name
