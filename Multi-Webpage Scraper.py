            ...
            if re.search(('topic'), link) != None and re.search(('getLastComment'), link) == None:
                ...
                for i in keywords:
                    if i in link:
                        ...
                        break
        # I extend nj_urls instead of just using a set on all the subforum topics so that the
        # order of the topics is mostly preserved in file explorer (the only possible time it wouldn't
        # be preserved is when there are >=2 relevant topics on a webpage of the subforum)
        nj_urls.extend(temp_urls)
    return nj_urls
